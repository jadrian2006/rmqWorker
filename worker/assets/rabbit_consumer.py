...
class rabbitConsumer(object):
    ...
    def requeue_message(self, body, delivery_tag):
        ...
        if REQUEUE_COUNT == 0:
            log.info('Requeued failed request for retry #%s', self.priority)
    ...
    def t_spawn_on_message(self, body, delivery_tag):
        ...
        log.info('Delivery Tag information %s', delivery_tag)
    ...
    def choose_source(self, body, delivery_tag):
        if 'deluge' in body[1]:
            log.info('We are going to process a deluge request')
            parms = ["",body[1]['deluge']['torrent_id'],body[1]['deluge']['torrent_name'],body[1]['deluge']['path']]
            log.info('Our parameters are %s', parms)
            try:
                delugePost.main(parms)
                return self.acknowledge_message(delivery_tag)
            except:
                log.error('Deluge client is not responding, requeing message and waiting 10 seconds')
